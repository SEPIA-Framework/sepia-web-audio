...
var PorcupineModule = (function() {
    ...
    return (
        function(wasmFileArrayBuffer, defaultWasmBinaryFile) {
            ...
            var read_, readAsync, readBinary, optimizedBinaryReadPromise, setWindowTitle;
            var nodeFS;
            var nodePath;
            if (ENVIRONMENT_IS_NODE) {
                ...
                if (scriptDirectory.indexOf("blob:") !== 0) {
					...
					if (typeof SepiaFW == "object" && SepiaFW.files) {
						console.error('SepiaFW optimizations');									//DEBUG
						optimizedBinaryReadPromise = function(url){
							console.error('optimizedBinaryReadPromise URL: ' + url);			//DEBUG
							return new Promise(function(resolve, reject) {
								function onsuccess(arraybuffer){
									console.error('optimizedBinaryReadPromise success');		//DEBUG
									resolve(arraybuffer);
								}
								function onerror(e){
									console.error('optimizedBinaryReadPromise error: ' + e);	//DEBUG
									reject(e);
								}
            ...
            function getBinaryPromise() {
				...
				}else if (optimizedBinaryReadPromise) {
					return optimizedBinaryReadPromise(wasmBinaryFile)
